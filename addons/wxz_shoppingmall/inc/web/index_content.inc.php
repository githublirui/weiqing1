...
$index_info_sql = "SELECT * FROM " . tablename('wxz_shoppingmall_index') . " WHERE uniacid={$_W['uniacid']} AND type=$type";
...
if (!$index_info) {
    ...
    for ($i = 1; $i <= $banner_num; $i++) {
        if ($index_info["banner{$i}"]) {
            $index_info["banner{$i}_arr"] = explode(',', $index_info["banner{$i}"]);
        }
    }
