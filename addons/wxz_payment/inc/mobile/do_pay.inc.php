...
if (!$order_id) {
    message('订单参数错误', $this->createMobileUrl('pay'));
}

$order_info = Order::getOrderById($order_id);

if (!$order_info || $order_info['status'] != 1) {
    message('订单不存在或已处理', $this->createMobileUrl('pay'));
}
...
if (!$user) {
    message('订单用户错误', $this->createMobileUrl('pay'));
}
...
$input->SetBody($title);
$input->SetAttach("wxz_payment|{$_W['uniacid']}");
