...
if(empty($category)){
	...
	$live_lists = pdo_fetchall("SELECT `id`,'rid' FROM ".tablename('wxz_wzb_live_setting')." WHERE uniacid=:uniacid AND cid=:cid",array(':uniacid'=>$uniacid,':cid'=>$id));
	if(!empty($live_lists) && is_array($live_lists)){
		foreach($live_lists as $row){
			pdo_delete('wxz_wzb_comment',array('rid'=>$row['rid'],'uniacid'=>$uniacid));
			pdo_delete('wxz_wzb_live_menu',array('rid'=>$row['rid'],'uniacid'=>$uniacid));
