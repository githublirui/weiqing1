...
$list = pdo_fetch("SELECT * FROM ".tablename('wxz_wzb_live_setting')." WHERE uniacid=:uniacid AND id=:id",array(':uniacid'=>$uniacid,':id'=>$id));
...
if (checksubmit('submit')) {
	...
	if(!empty($id)){
		pdo_update('wxz_wzb_live_setting',$data,array('id'=>$id,'uniacid'=>$uniacid));
