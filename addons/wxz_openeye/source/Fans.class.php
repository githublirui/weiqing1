...
class Fans {
    ...
    public function save($entity) {
        ...
        $sql = 'SELECT * FROM ' . tablename('wxz_openeye_fans') . ' WHERE `uniacid`=:uniacid AND `openid`=:openid';
        $pars = array();
        ...
        if (!empty($exists)) {
            ...
            $ret = pdo_update('wxz_openeye_fans', $rec, $filter);
            if ($ret !== false) {
                return $exists['uid'];
            } else {
        ...
        $ret = pdo_insert('wxz_openeye_fans', $rec);
        if (!empty($ret)) {
            return pdo_insertid();
        } else {
    ...
    public function remove($uid, $isOpenid = false) {
        ...
        $del = pdo_delete('wxz_openeye_fans', $pars);
        if ($del !== false) {
            return true;
        } else {
    ...
    public function getOne($uid, $isOpenid = false) {
        ...
        if ($isOpenid) {
            ...
            $pars[':uid'] = intval($uid);
            $sql = 'SELECT * FROM ' . tablename('wxz_openeye_fans') . ' WHERE `uniacid`=:uniacid AND `uid` =:uid';
        }
    ...
    public function getAllAdmin() {
        ...
        $pars = array();
        $pars[':uniacid'] = $_W['uniacid'];
        $sql = 'SELECT * FROM ' . tablename('wxz_openeye_fans') . ' WHERE `uniacid`=:uniacid AND is_admin=1';
    ...
    public function getAll($filters = array(), $pindex = 0, $psize = 15, &$total = 0) {
        ...
        $sql = 'FROM ' . tablename('wxz_openeye_fans') . ' AS `f` LEFT JOIN ' . tablename('mbrp_records') . ' AS `r` ON (`f`.`uid`=`r`.`uid`) WHERE ';
        $sql .= $condition;
        if ($pindex > 0) {
